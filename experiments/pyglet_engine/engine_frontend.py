...
class Sprite:
    ...
    def load_images(self):
        ...
        images['stand_left'] = frontend.Image.load('marek-left.png')
        images['stand_right'] = frontend.Image.load('marek-right.png')
...
class Bullet:
    def __init__(self, x, y, dir):
        ...
        self.timer = 0
        self.image = frontend.Image.load('bullet.png')
...
def process_tile_data(grid, layout):
    ...
    map_image = frontend.Image.create(width, height)
    i, j = 0, 0

    for y in layout:
        for x in y:
            map_image.blit_into(grid[int(x) - 1], i * tile_w, j * tile_h)
            i += 1
        i = 0
        j += 1
...
def load_map(filename):
    ...
    tileset_image = frontend.Image.load(tileset_name)
    tile_grid = tileset_image.get_grid(tile_w, tile_h)
