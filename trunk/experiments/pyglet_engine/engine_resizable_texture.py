...
if __name__ == '__main__':
    ...
    window = pyglet.window.Window(width=map.win_w*2, height=map.win_h*2,
                                  resizable=True)

    # Don't allow the window to become smaller than the original map size,
    # otherwise the scene starts getting clipped since you can't have a
    # glViewport() larger than the window itself.
    window.set_minimum_size(map.win_w, map.win_h)
    ...
    # Texture to hold rendered camera view at original resolution
    texture = pyglet.image.Texture.create_for_size(GL_TEXTURE_2D, 
        map.win_w, map.win_h, GL_RGB)
    texture = texture.get_region(0, 0, map.win_w, map.win_h)

    def update(dt):
        # BUGS: Some GL exception may get thrown when we exit the game
        # Maybe update() gets called after the window was already destroyed?
        
        # Setting glOrtho() AND glViewport() to the map size will cause the
        # map to be rendered at its original size into the lower left corner
        # of the window. Since we're using double buffering, this temporary
        # rendering will never be seen
        glMatrixMode(GL_PROJECTION)
        glLoadIdentity()
        glOrtho(0, map.win_w, 0, map.win_h, -1, 1)
        glMatrixMode(GL_MODELVIEW)
        glViewport(0, 0, map.win_w, map.win_h)

        # Blit the camera view 
        camera.update()

        # Copy the temporarily rendered camera view to the texture
        buffer = pyglet.image.get_buffer_manager().get_color_buffer()
        texture.blit_into(buffer, 0, 0, 0)

        # Set projection and viewport back to full window size
        glMatrixMode(GL_PROJECTION)
        glLoadIdentity()
        ...
        # Blit the previously rendered camera view scaling it to match the
    ...
    pyglet.clock.schedule(update)  # Run update on vertical retrace
    pyglet.app.run()
